...
func (t Task) String() string {
	return fmt.Sprintf("%s.%s.", t.Host, t.Domain)
}
var TaskQueue = make(chan *Task, 1)
var ResultQueue = make(chan *Result, 1)
...
func (r Result) GetHostname() string {
      ...
      case task, more := <-w.TaskChannel:
          ...
          if more {
            ...
            for _, ans := range r {
              ...
              if task.Wildcard.IP.Equal(ansIp) {
                continue
              }
              res.reverseLookup()
...
func CheckWildcard(task Task) Result  {
  randInt := 1234567
